...
│   ├── pipelines.py          # PostgreSQL Pipeline（upsert + ID 缓存 + 13 种 item 处理）
│   ├── settings.py           # 动态加载 config JSON
│   ├── utils/
│   │   ├── config_loader.py  # get_delay/spider/database/minio_config()
│   │   ├── delay.py          # 专辑间延迟控制器
│   │   └── minio.py          # download_and_upload() — 下载 + 上传 MinIO
│   └── spiders/
│       ├── album_test.py         # 测试爬虫（10 张专辑）
│       ├── album_bulk.py         # 批量爬虫（full/incremental + max_albums）
│       ├── album_incremental.py  # 增量爬虫（遇完整专辑即停）
│       ├── circle_members.py     # 社团成员
...
**流程（parse_album_detail）：**
1. 构造 AlbumItem → yield
2. 遍历曲目 `<li data-audio="...">` → `download_and_upload()` 下载+上传 MinIO → yield WorkFileItem
...
7. 专辑间延迟：`between_albums_min + rand(0, between_albums_random)` 秒

**album_bulk / album_incremental / album_test 均集成 MinIO 音频下载。**
...
- 从 circles 表读全部社团，逐社团爬 `/l/{name}/`
- `full`: 全部重爬 / `incremental`: 跳过已有成员的社团
- 每社团间延迟：`download_delay` 秒

### user_roles — 用户角色

- 爬 `/setup`，解析 STAFF / PRO 区域
- `full`: 全部 UPDATE role / `incremental`: 仅 UPDATE role 为空或 normal 的用户
- 直接 DB UPDATE，不走 Pipeline
...
- `full`: 忽略 userpage_crawled_at / `incremental`: 跳过 < 30 天的
...
**临时字段解析：** Pipeline `_resolve_refs` 将 `_dizzylab_id` / `_dizzylab_user_id` / `_dizzylab_labelid` / `_tag_name` 等临时字段通过内存缓存映射为 DB 主键。

## MinIO 音频下载

`utils/minio.py::download_and_upload(cdn_url, album_slug, sort_order)` 在专辑爬虫的 parse_album_detail 中被调用：
...
- `users` — dizzylab_user_id UNIQUE, user_role: normal/pro/staff, userpage_crawled_at
...
```bash
psql -U postgres -d indietracks -f database/create_database.sql   # 建表
